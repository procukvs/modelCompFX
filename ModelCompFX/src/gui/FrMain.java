...
public class FrMain extends BorderPane {
	...
	public FrMain(Stage stMain, DbAccess db){
		...
		MenuItem machine = new MenuItem(Model.title("Machine", 1));  		
		MenuItem algorithm = new MenuItem(Model.title("Algorithm", 1));	
		...
		mFile.getItems().addAll(input,output); 
		...
		label.setStyle("-fx-font: italic bold 20px 'Arial'"); 
		HBox menu = new HBox();
		Button exit = new Button("Вийти з програми");
		menu.getChildren().addAll(menuBar,exit);
		
		computer.setOnAction(e-> DialogWork.showAlertInform("Model", "Computer").showAndWait());
		machine.setOnAction(e->env.set("Machine"));
		algorithm.setOnAction(e->env.set("Algorithm")); //(e-> DialogWork.showAlert("Model", "Algorithm"));
		...
		bxNorth.getChildren().addAll(menu,label);  
	...
	public void setModel(String type, int id) {
		// викликається при обробці пунктів меню Системні дії!!!
		/*
		//boolean files = type.equals("Input") || type.equals("Output");
...
/*
lambda.setOnAction(e->Alert.create()
.owner(stMain)
