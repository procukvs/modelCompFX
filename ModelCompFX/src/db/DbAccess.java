	...
	*/
	...
	public int getModelCount(String type) {
		...
		try{
			sql = "select count(*) from " + tableModel(type) + 
			      " where section = '" + Parameter.getSection() + "'";
			s.execute(sql);
			rs = s.getResultSet();
	...
	public ArrayList <String> getAllModel(String type){
		...
		sql = "select name from " + tableModel(type) + 
			  " where section = '" + Parameter.getSection() + "' order by name";
		try{ 
			s.execute(sql);
	        ResultSet rs = s.getResultSet();
	        while((rs!=null) && (rs.next())) {
		...
		try{ 
			...
			for (int j = 0; j < sl.size(); j++){
				...
				for (int i = 0; i < select.length; i++ ){
					...
					while((rs!=null) && (rs.next())) {
						row = new ArrayList();
						if (Parameter.getRegime().equals("teacher")) row.add(section);
	...
	public boolean isModel(String type, String name){
    	try{ int cnt = 0;
		     sql = "select count(*) from " + tableModel(type) + 
		    	   " where section = '" + Parameter.getSection() + "' and name = '" + name +"'";
             s.execute(sql);
             rs = s.getResultSet();
             if((rs!=null) && (rs.next()))cnt = rs.getInt(1);
             return (cnt > 0);
		...
		case "Calculus": 
			sql = "select name from " + tableCommand(type) +
				   " where idModel = (select id from " + tableModel(type) + " where name = '" + nmSet + "'" +
				                      " and section = '" + Parameter.getSection()+ "')" ; //+
				  // " order by num";
	...
	public void setParameters(){
		...
		try{ 
	        ...
	        while((rs!=null) && (rs.next())) {
	    		...
	    		case "Section": Parameter.setSection(rs.getString(2)); break;
	    		case "PostVar" : Parameter.setPostVar(rs.getString(2)); break;
	    		case "RecurSubst" : Parameter.setRecurSubst(rs.getString(2)); break;
	    		case "Version" : Parameter.setVersion(rs.getString(2)); break;
